...
import { sessionWorkingDirectory, setSessionHome } from '@deepseek-ai/dsh-sandbox-policy'
...
/**
 * Host `session.rehome` on the Session Controller, when the composition mounts it.
 * The deleted host-apiproxy `ctx.apiProxy.sessions.rehome` envelope is gone.
 * @param ctx plugin context
 * @returns the controller method, or undefined
 */
function hostRehome(ctx) {
  const controller = ctx.get('sessionController')
  const rehome = controller?.rehome
  return typeof rehome === 'function' ? rehome.bind(controller) : undefined
}
...
async function rehome(ctx, session, canonical) {
  ...
  if (host !== undefined) {
    try {
      return await host({ sessionId: session.id, path: canonical })
    } catch (error) {
      throw new Error(error instanceof Error ? error.message : `session.rehome failed for ${canonical}`)
    }
  }
